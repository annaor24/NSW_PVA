...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("female", "All female?", choices = c("Yes", "No")),
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    x_values <- as.numeric(colnames(df)[3:72])
    y_values <- as.numeric(df[1, 3:72])
    ...
    ggplot(data = df_plot, aes(x = x, y = y)) +
      ...
      geom_line() +
      geom_vline(xintercept = first_below_0.5, linetype = "dashed", color = "red") +
      annotate("text", x = first_below_0.5, y = 0.5, label = first_below_0.5_column,
               color = "red", vjust = -15, hjust = -0.3, size = 4) +
      geom_vline(xintercept = 2017, color = "dark grey") +  # Black x-axis line
      geom_hline(yintercept = 0, color = "dark grey") +     # Black y-axis line
  ...
  output$summary_table <- renderTable({
    ...
    summary_df <- data.frame("2025" = df[1,12],"2050" = df[1, 37], "2085" = df[1, 72])
    colnames(summary_df) <- c("2025", "2050", "2085")
